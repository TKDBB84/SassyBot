...
const quoteFunction = message => {
  if (hasSingleMention(message)) {
    ...
    activeChannel
      .fetchMessages({ limit: 50, before: message.id })
      .then(messages => {
        ...
        for (let i = 0, iMax = messagesWithReactions.length; i < iMax; i++) {
          messagesWithReactions[i].reactions.forEach(reaction => {
            if (!foundOne) {
...
const rQuoteFunction = message => {
  if (
    ...
    if (!parts) {
      getQuotesByUser.all(
        [message.guild.id, quotedMember.id],
        (error, rows) => {
      ...
      getQuotesByUser.all(
        ...
        (error, rows) => {
          ...
          for (let i = 0, iMax = rows.length; i < iMax; i++) {
            ...
            if (!row.quote_text || row.quote_text === "") {
              fetches.push(
                client.channels.get(row.channel_id).fetchMessage(row.message_id)
              );
            } else {
    ...
    message.channel.send("You must specify whose quote you want to retrieve", {
      disableEveryone: true
    });
  }
};
