...
import {
  AbsentOrPromoteFunctions,
  resumeAbsentOrPromote
} from './AbsentPromote';
import {
  newMemberJoinedCallback,
  newMemberListener,
  setNewUserWorkflow
} from './NewUserManager';
...
const importedFunctions: SassyBotImportList = [
  DiceFunctions,
  QuoteFunctions,
  AbsentOrPromoteFunctions
];
...
const getDisplayName: (message: Message) => string = function(
  message: Message
): string {
...
const shiftyEyes: SassybotTrollCommand = function shiftyEyes(
  message: Message
): boolean {
  let outMessage = '';
  const leftEyes = /.*<(\s*.\s*)<.*/;
...
const processPleaseStatement: SassybotTrollCommand = (
  message: Message
): boolean => {
...
const isSassyBotCall: (message: Message) => boolean = function(
  message: Message
): boolean {
  return (
...
const spamFunction: SassyBotCommand = (message: Message): void => {
  const author_id = getAuthorId(message);
  if (
    author_id === Users.Sasner.id ||
    author_id === Users.Verian.id ||
    author_id === Users.Tyr.id
  ) {
    ...
    sassybotRespond(
      message,
      'This functionality is limited to Sasner & server owners'
    );
  }
};
...
const processSassybotCommand: (
  message: Message
) => void = function processSassybotCommand(message: Message): void {
...
const messageEventHandler: (message: Message) => void = (
  message: Message
): void => {
    ...
    if (author_id !== Users.Sasner.id) {
      for (let i = 0, iMax = preProcessTrollFunctions.length; i < iMax; i++) {
        ...
        if (random_number < preProcessTrollFunctions[i].chance) {
          const continueProcessing = preProcessTrollFunctions[i].process(
            message
          );
