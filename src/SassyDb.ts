...
class SassyDb {
  ...
  public constructor() {
    ...
    this.stmtRemoveSpamChannel = this.connection.prepare('DELETE FROM spam_channels WHERE guild_id = :guild_id;');
    this.stmtGetSpamChannels = this.connection.prepare('SELECT * FROM spam_channels');
  ...
  public getSpamChannelMap: () => Map<string, string> = (): Map<string, string> => {
  ...
  public addSpamChannel: (guildId: string, channelId: string) => void = (guildId: string, channelId: string): void => {
    this.stmtAddSpamChannel.run({
      channel_id: channelId,
      guild_id: guildId,
    });
  };
  public removeSpamChannel: (guildId: string) => void = (guildId: string): void => {
