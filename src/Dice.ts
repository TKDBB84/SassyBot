 ...
 */
const parseDice: (message: Message) => {num: number, sides: number} = (message: Message): {num: number, sides: number} => {
 ...
 * @returns array
 */
const rollDice: (parsedDice: {num: number, sides: number}) => number[] = (parsedDice: {num: number, sides: number}): number[] => {
 ...
 */
const parseKeepOrDrops: (message: Message) => {drop: boolean, keep: boolean, numDice: number} = (message: Message): {drop: boolean, keep: boolean, numDice: number} => {
    const parsedMessage = message.content.split(" ")[2];
...
const actionKeepOrDrops: (keepOrDrop: {drop: boolean, keep: boolean, numDice: number}, rolls: number[]) => {dropped: number[], kept: number[]} = (keepOrDrop: {drop: boolean, keep: boolean, numDice: number}, rolls: number[]): {dropped: number[], kept: number[]} => {
 ...
 * @param arr
 */
const shuffle: (arr: number[]) => number[] = (arr: number[]): number[] => arr
    .map(a => [Math.random(), a])
    .sort((a, b) => a[0] - b[0])
    .map(a => a[1]);
 ...
 */
const parseStaticAdditions: (message: Message) => {minus: boolean, constant: number, plus: boolean} = (message: Message): {minus: boolean, constant: number, plus: boolean} => {
    const parsedMessage = message.content.split(" ")[2];
